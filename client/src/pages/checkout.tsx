...
export default function Checkout() {
  ...
  useEffect(() => {
    apiRequest("POST", "https://checkout-api-pcz0.onrender.com/api/create-payment-intent", {
      customerName: "Placeholder",
      customerEmail: "placeholder@the15minutescoach.com",
...
const [errorMessage, setErrorMessage] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const returnUrl = `${window.location.origin}/thank-you?payment_intent=${paymentIntentId}&email=${encodeURIComponent(email)}&name=${encodeURIComponent(name)}`;

    try {
      await apiRequest("POST", "https://checkout-api-pcz0.onrender.com/api/create-payment-intent", {
        customerEmail: email,
        customerName: name,
        productId: "narcissism-survival-guide"
      });
    } catch (e) {
      console.warn("Non-blocking error updating payment intent:", e);
    }
